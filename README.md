...
### 全新仪表盘体验
- 首页入口升级为品牌化欢迎页，展示累计行程、活动数量与下一段旅程的 AI 摘要，点击即可进入真实应用感的仪表盘。
- 「当前规划」模块以图像化卡片呈现即将出发的行程、预算估算、节奏指数以及即时提醒列表，可一键跳转到工作台、优化顺序或添加活动。
- 「Outlook 风格日历」模块提供未来两周的紧凑栅格视图，对应行程活动会以标签和时间标记显示，让用户在繁忙日程中快速识别重点。
- 「AI 行程优化中心」整合节奏、预算、交通模式等指标，并内嵌轻量化 AI 表单，实现生成、评估、再规划的一站式流程。
- 「下一步选择」模块根据 AI 推荐指数排序候选行程，提供设为当前、跳转详情等决策动作，打造贴近日常办公的多模块工作区。

### 外部 API 接入点参考
- **日历与提醒**：Dashboard 的日历模块与同步按钮可连接 Microsoft Graph Calendar API、Google Calendar API 或 iCalendar ICS，用于推送与订阅事件。
- **地图与交通评估**：节奏/交通指标可结合 Google Maps Directions API、高德/百度地图出行 API 计算里程与耗时，进一步优化出行路径。
- **价格与预算**：预算估算可接入 Amadeus、Skyscanner、携程开放平台等价格数据源，实现住宿、机票、交通费用的实时评估。
- **AI 优化**：AI 行程助手和智能提醒可对接 OpenAI、Azure OpenAI、Claude 等大型模型服务，并结合自建的行程知识库进行检索增强。
- **视觉与素材**：首页与当前规划模块可引用 Unsplash、Pexels 等图片服务或自定义插画 API 丰富视觉呈现。
